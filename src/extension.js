...
const { BROWSER_TOOL_METHODS } = require('./browser-tools.js');
...
class Session {
  constructor(id, projectRoot) {
    ...
    this.browser = null;
    // Chat-file write state. Writes are serialized per session and skipped when
    // nothing changed - see _writeChatFile for why both matter.
...
class NavyCoderViewProvider {
  ...
  // Two callers write this file: _persistCheckpoints, on a 500ms debounce after
  // EVERY file edit, and saveProjectSession, awaited at the end of each turn.
  // They could previously run at the same time against the same path, which is an
  // interleaving hazard, and each re-serialized the whole chat - every message
  // plus up to 8 MB of checkpoint text - however little had actually changed. On a
  // long session that is the same ever-growing payload written over and over.
  //
  // Writes are therefore serialized per session (one in flight, at most one
  // re-run queued behind it), and a write whose content is byte-identical to the
  // previous one is skipped. That last part removes most of the work in practice:
  // the end-of-turn save usually finds exactly what the checkpoint debounce just
  // wrote a moment earlier.
  _writeChatFile() {
    const s = this._session;
    if (!s._writeInFlight) return this._startChatWrite(s);
    // A write is already running. Chain exactly ONE follow-up behind it and hand
    // every caller that arrives meanwhile the same promise. Awaiting this has to
    // mean 'my content is on disk' - returning the in-flight promise instead
    // would resolve the end-of-turn save as soon as an earlier checkpoint write
    // finished, before the turn's own messages had been written at all.
    if (!s._writeQueued) {
      s._writeQueued = s._writeInFlight.catch(() => {}).then(() => {
        s._writeQueued = null;
        return this._startChatWrite(s);
      });
    }
    return s._writeQueued;
  }

  _startChatWrite(s) {
    s._writeInFlight = this._writeChatFileNow(s).finally(() => { s._writeInFlight = null; });
    return s._writeInFlight;
  }

  async _writeChatFileNow(s) {
    ...
    try {
      // Serialized WITHOUT the timestamp so it can be compared against the last
      // write - including `updated` would make every payload unique and defeat the
      // check entirely. The timestamp is spliced onto the front afterwards, so the
      // big structure is stringified exactly once either way.
      const body = JSON.stringify({ id: s.id, messages: s.messages, digest: s.sessionDigest || '', checkpoints: keepCps });
      if (body === s._lastWrittenBody) return;   // nothing changed since the last write
      s._updated = new Date().toISOString();
      const json = '{"updated":' + JSON.stringify(s._updated) + ',' + body.slice(1);
      await vscode.workspace.fs.writeFile(
        vscode.Uri.file(path.join(chatsDir, s.id + '.json')),
        Buffer.from(json, 'utf8')
      );
      // Recorded only after a write that actually succeeded, or a failed write
      // would be remembered as done and never retried.
      s._lastWrittenBody = body;
  ...
  async _askNavyTurn(prompt, includeContext, selectedModel, attachedFiles, images) {
    ...
    try {
      ...
      for (let iteration = 0; iteration < maxIterations; iteration++) {
        ...
        // The same lie, but from a turn that DID use tools — and by far the
        // common shape of it in practice: the model reads files, writes nothing,
        // then signs off with a confident "**Changed:** a.ts, b.ts". The guard
        // above cannot see it (it requires no tool calls at all), so this used to
        // reach the user as a finished-looking report with only a footnote under
        // it saying none of it happened. A warning is the wrong remedy for work
        // that was never done: send it back to actually do it, while the turn is
        // still open. Evidence, not vibes — Navy's own record of what the turn
        // changed — and the strict `Changed:`-names-a-file detector rather than
        // the loose prose one, which would misfire on a legitimate report.
        // A turn that ran a COMMAND is taken at its word: `git apply`, `sed -i`
        // or a codemod changes files with no write tool involved. Shares
...
mixinPrototype(NavyCoderViewProvider.prototype, BROWSER_TOOL_METHODS);

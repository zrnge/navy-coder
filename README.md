...
Type `/` in the composer for the built-in prompts — `/fix`, `/review`, `/test`, `/security`, `/audit` (supply-chain scan), `/playthrough` (visual browser test), `/commit` and a dozen more.
...
- **Two approval gates, not one** — `navy.approvalMode` covers changes to *files*: every edit is a diff you approve, and every change is contained to the workspace, checkpointed for undo, and visible in git afterwards. `navy.commandApproval` covers *execution*: shell commands, test runs, dev servers, background processes, third-party MCP tool calls, and launching the `/playthrough` browser. Both default to `ask-always` and each has to be turned off on its own, because they are not the same decision — a file edit is bounded and reversible, whereas Navy cannot know what a command will do before it runs or take it back afterwards. Until 0.3.1 these were one setting, so switching off diff prompts also granted unattended command execution; if you had `auto-approve` set before upgrading, file edits stay automatic and commands go back to asking.
- **Opt-in sandboxing** (`navy.sandboxMode`) — a second, independent layer under the approval gate, and it never degrades quietly: whichever backend is selected, if it cannot proceed the command is refused rather than run unsandboxed, because a sandbox that switches itself off is worse than none. `docker` refuses to guess at a generic image if the project has no devcontainer/Dockerfile of its own, and targets the container rather than the host, so commands are written and run in POSIX `sh` even from Windows. `wsl` is that same container isolation through [WSL Containers](https://devblogs.microsoft.com/commandline/wsl-container-is-now-available-for-public-preview/) (`wslc`) — Microsoft's Linux-container runtime built into Windows — so Windows finally gets real container sandboxing **without Docker Desktop** (needs WSL 2.9.3+). `native` wraps the host's own shell instead — writes confined to the project and temp, reads of `~/.ssh`, `~/.aws`, `~/.gnupg`, `~/.kube`, `~/.netrc`, gcloud and docker credentials blocked — and it does **not** restrict the network, because blocking it would break `npm install` and every other command people actually want sandboxed. `~/.npmrc` and `~/.gitconfig` stay readable for the same reason.
- **The playthrough browser is contained** — `/playthrough` starts a real Chrome, so it is treated as execution, not as a page view. The launch goes through `navy.commandApproval` (the same gate as shell commands), asked once per browser rather than once per click, because a browser hands the model navigation plus arbitrary in-page JavaScript. Each run gets a fresh throwaway `--user-data-dir`, so it never reads your real cookies, sessions or history; Chrome's own OS sandbox is left **on** — Navy never passes `--no-sandbox` — and only `http(s)` URLs are reachable, so a page cannot talk the browser into opening `file://` or a privileged `chrome://` surface. The browser belongs to the chat that opened it and dies with it
...
| `navy.sandboxMode` | `off` | `docker` (container, any OS), `wsl` ([WSL Containers](https://devblogs.microsoft.com/commandline/wsl-container-is-now-available-for-public-preview/) via `wslc` — container isolation on Windows with **no Docker Desktop**; needs WSL 2.9.3+), `native` (sandbox-exec / bubblewrap; macOS and Linux only), or `off`. See [Safety](#safety) |
| `navy.sandboxImage` | `` | Default container image for `docker`/`wsl` sandboxing, for **every** project that has no devcontainer/Dockerfile of its own (e.g. `node:20`) — set it in **User** settings once instead of per repo. A project's own config always wins; a workspace override still scopes it to one project |
| `navy.chromePath` | *(empty)* | Browser executable `/playthrough` drives. Empty auto-detects Chrome, then Edge, then Chromium in their standard locations |
| `navy.browserHeadless` | `false` | Run the `/playthrough` browser without a visible window. Off by default so you can watch it work; either way it uses a fresh throwaway profile |
| `navy.shell` | `auto` | Shell for `run_command`/`run_tests`/`run_project`, and the dialect Navy writes for: `auto`, `cmd`, `powershell`, `pwsh`, `sh`, `bash`. Ignored under a container sandbox (`docker`/`wsl`), which always targets `sh` |
...
| `navy.sandboxMode` | `off` | `off`, `docker`, `wsl` (Windows, no Docker Desktop) or `native` — see [Safety](#safety) |
